...
def decide_search_type(topic: str, choice_names: List[str] = ['neural', 'keyword']):
...
def create_keyword_query_generation_prompt(topic: str, n: int):
...
def create_neural_query_generation_prompt(topic: str, n: int):
    return f"""I'm writing a research report on {topic} and need help coming up with Exa keyword search queries.
...
def generate_search_queries(topic: str, n: int, searchType: str):
    if(searchType != 'keyword' and searchType != 'neural'):
        raise ValueError('searchType must be either "keyword" or "neural"')
...
def get_search_results(queries: List[str], type: str, linksPerQuery: int = 2):
    results = []
    for query in queries:
        search_response = exa.search(query, type=type, num_results=linksPerQuery, use_autoprompt=False)
        results.extend(search_response.results)
    return results

def get_page_contents(search_results):
    contents_response = exa.get_contents(search_results)
